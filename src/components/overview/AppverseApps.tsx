// Appverse Apps Grid v3.4.0 - App launcher with categories, search, expandable cards
...
export function AppverseApps() {
  ...
  const counts = useMemo(() => {
    ...
    categoryTabs.forEach(tab => {
      if (tab.key !== 'all') {
        result[tab.key] = APPVERSE_APPS.filter(a => a.category === tab.key).length;
      }
    });
    return result;
  }, []);
                  ...
                  <div className="px-4 pb-4 pt-0 border-t border-border/30 space-y-3">
                    ...
                    {app.healthConfig && (
                      <div className="flex items-center gap-4 text-xs text-muted-foreground">
                        {app.healthConfig.localPort && (
                          <div className="flex items-center gap-1">
                            <Activity className="w-3 h-3" />
                            <span>Port {app.healthConfig.localPort}</span>
                          </div>
                        )}
                        {app.healthConfig.requiredServices && (
                          <div className="flex items-center gap-1">
                            <span>Services:</span>
                            {app.healthConfig.requiredServices.map(s => (
                              <span key={s} className="badge badge--muted text-[9px]">{s}</span>
                            ))}
                          </div>
                        )}
                      </div>
                    ...
                    <div className="flex flex-wrap gap-2 pt-2">
                      ...
                      {app.urls.production && (
                        <Button
                          size="sm"
                          variant="default"
                          className="h-7 text-xs gap-1.5 bg-signal-green hover:bg-signal-green/90"
...
export { AppverseApps as EchoverseApps };
