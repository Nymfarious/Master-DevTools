...
export function SystemHealthMonitor() {
  ...
  const checkAll = async () => {
    const services = Object.keys(systemHealth) as Array<keyof typeof systemHealth>;
  ...
  return (
    <section className="space-y-3">
      <div className="flex items-center justify-between">
        ...
        >
          <RefreshCw className={cn("w-3 h-3", Object.values(checking).some(Boolean) && "animate-spin")} />
          Check All
        </Button>
      </div>
      
      <div className="grid grid-cols-3 gap-3">
        {(Object.entries(systemHealth) as Array<[keyof typeof systemHealth, typeof systemHealth.api]>).map(([key, value]) => {
          ...
          return (
            ...
            >
              ...
              <p className={cn(
                ...
                {isChecking ? 'Checking...' : value.status === 'healthy' ? 'Healthy' : value.status}
              </p>
